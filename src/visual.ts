...
function drawVect(vect: vects.vector, label: string) {
    c.ctx.beginPath();
    ...
    c.ctx.fillText(label, 
        vects.coordToPix(vect)[0] + s.TEXT_HEIGHT*vect[0]/len - s.TEXT_HEIGHT*label.length/4, 
        vects.coordToPix(vect)[1] + s.TEXT_HEIGHT*vect[1]/len + s.TEXT_HEIGHT/2);
}
...
function name(concat: string): string{
    let out = ""
